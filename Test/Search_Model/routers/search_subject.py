...
async def generate_search_project(request: SearchshimilerResponse):
    try :
        # 데이터 준비
        ...
        # 요청에서 데이터 추출
        input_text = request.project_info['problem_solving']['solutionIdea']
        target_stack = request.project_info['technology_stack']
        top_k = request.top_k
        ...
        # 키 오류 처리
        return SearchshimilerResponse(
            similer_ID={"project_ID": [], "error": f"Missing key: {str(e)}"}
        )

    except Exception as e:
        # 기타 오류 처리
        return SearchshimilerResponse(
            similer_ID={"project_ID": [], "error": str(e)}
        )